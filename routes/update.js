...
router.get("/", ensureAuthenticated, (req, res) => {
	db.query(`SELECT patient.patientID, patient.fName, patient.lName, additionalitems, description, difficulty, latitude, longitude, name, objectives, reward, status, type FROM quests JOIN users_patients AS patient ON quests.patientID = patient.patientID WHERE quests.doctorID = ?`, [req.user.doctorID], (err, results) => {
		if (err) console.log(err);
		res.render("update", { user: req.user, patients: results });
	});
});
